...
# Bagus comment:definition of UserProfile, I modified this Michael model a bit and following an example from https://docs.djangoproject.com/en/1.4/topics/auth/#storing-additional-information-about-users 
#I noticed that if we ran the previous model it will create another user table on the database. 
...
#Below is the previous model added by Michael
"""
# Create your models here.
# Bagus comment: I think we don't need to create another table for user since Django already provide it as a default
class User(models.Model):
    username = models.ForeignKey('auth.User', related_name='user')#models.CharField('Username', max_length=30, unique=True, help_text="Required. 30 characters or fewer. Alphanumeric characters only (letters, digits and underscores).")
...
#Bagus comment: I move this to above and define a new class name "UserProfile" based on an example from Django document to store additional information into user table.
class Profile(models.Model):
    user = models.ForeignKey('auth.User', related_name='user')#OneToOneField
    preferred_name = models.CharField('Preferred name', max_length=30, help_text='Supports Unicode and permits names to be displayed however the user sees fit within reasonable bounds. For example, first="Yamato"+last="Takahashi" could have "é«˜æ©‹ å¤§å’Œ" for his preferred name. The preferred name shall appear preferentially in the profile page. The first/last name shall be used for URLs, core database, and menu selections, with the preferred name appearing as a secondary element.')
    dob = models.DateField('Date of Birth', null=True, blank=True)
    GENDERS = (
        (0, 'No answer'),
